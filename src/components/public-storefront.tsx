...
export function PublicStorefront({ catalog, catalogUnavailable = false }: { catalog: PublicCatalog; catalogUnavailable?: boolean }) {
  ...
  function addLine(line: CartLine, drinks: Array<{ product: PublicProduct; quantity: number }> = []) {
    setCart((current) => {
      const next = editing ? current.map((item) => item.key === editing.line.key ? { ...line, key: editing.line.key } : item) : [...current, line];
      return drinks.reduce<CartLine[]>((items, { product, quantity }) => {
        if (!quantity) return items;
        const existing = items.find((item) => item.kind === "sale_product" && item.id === product.id);
        if (existing) return items.map((item) => item.key === existing.key ? { ...item, quantity: item.quantity + quantity } : item);
        return [...items, { key: cartKey(`product:${product.id}`), kind: "sale_product", id: product.id, name: product.name, image_url: product.image_url, quantity, unit_price_cop: product.sale_price_cop, additions: [], removed_components: [], presentation: presentation(product) }];
      }, next);
    });
...
function PizzaConfigurator({ catalog, existing, initial, onAdd, onClose }: { catalog: PublicCatalog; existing: CartLine | null; initial: PublicPizzaPrice; onAdd: (line: CartLine, drinks?: Array<{ product: PublicProduct; quantity: number }>) => void; onClose: () => void }) {
  ...
  const [personalizationView, setPersonalizationView] = useState<"remove" | "extras">("extras");
  const [includeBeverageStep, setIncludeBeverageStep] = useState(false);
  ...
  const personalizationStep = mode === "half" ? 4 : 3;
  const baseReviewStep = mode === "half" ? 5 : 4;
  const beverageStep = includeBeverageStep ? baseReviewStep : null;
  const reviewStep = baseReviewStep + (includeBeverageStep ? 1 : 0);
  const totalSteps = reviewStep;
  const availableDrinks = catalog.products.filter((product) => product.available);
  const selectedDrinks = availableDrinks.flatMap((product) => {
    const quantity = drinkQuantities[product.id] ?? 0;
    return quantity ? [{ product, quantity }] : [];
  });
  const drinksTotal = selectedDrinks.reduce((sum, { product, quantity }) => sum + product.sale_price_cop * quantity, 0);
  ...
  const totalToCart = unitPrice * quantity + drinksTotal;
  const isPersonalizationStep = step === personalizationStep;
  const isBeverageStep = beverageStep === step;
  const isReview = step === reviewStep;
  const title = step === 1 ? "Tamano" : step === 2 ? "Tipo" : mode === "half" && step === 3 ? "Segundo sabor" : isPersonalizationStep ? "Personaliza" : isBeverageStep ? "Bebidas" : "Resumen";
  ...
  function changeDrink(product: PublicProduct, delta: number) { setDrinkQuantities((current) => ({ ...current, [product.id]: Math.max(0, (current[product.id] ?? 0) + delta) })); }
  function finishPizza() { onAdd({ key: cartKey(`pizza:${selected.id}`), kind: "pizza", id: selected.id, secondary_id: second?.id, name: selected.flavor_name, secondary_name: second?.flavor_name, size_name: selected.size_name, image_url: selected.image_url, quantity, unit_price_cop: Math.max(selected.sale_price_cop, second?.sale_price_cop ?? 0), additions, removed_components: removed }, selectedDrinks); }
  function openBeverageStep() { setIncludeBeverageStep(true); setStep(baseReviewStep); }
  function renderStep() {
    ...
    if (isPersonalizationStep) return <div className="public-personalization"><div className="public-personalization-tabs" role="tablist"><button aria-selected={personalizationView === "extras"} className={personalizationView === "extras" ? "selected" : ""} onClick={() => setPersonalizationView("extras")} role="tab" type="button">Agregar extras</button><button aria-selected={personalizationView === "remove"} className={personalizationView === "remove" ? "selected" : ""} onClick={() => setPersonalizationView("remove")} role="tab" type="button">Quitar ingredientes</button></div>{personalizationView === "extras" ? compatibleAdditions.length ? <section className="public-personalization-section public-personalization-panel public-extras-section"><div className="public-section-title"><span>Personaliza a tu gusto</span><h3>Agregar ingredientes extras</h3></div><div className="public-additions">{compatibleAdditions.map((addition) => { const selectedScope = mode === "half" ? scopes[addition.id] ?? "whole" : "whole"; const current = additions.find((item) => item.id === addition.id && item.scope === selectedScope)?.quantity ?? 0; return <div key={addition.id}>{addition.image_url ? <img alt="" src={addition.image_url}/> : <span className="public-addition-fallback">+</span>}<span>{addition.name}<small>+{formatCop(addition.price_cop)}</small></span>{mode === "half" ? <div className="public-scope-choice"><button className={selectedScope === "whole" ? "selected" : ""} onClick={() => selectAdditionScope(addition, "whole")} type="button">Toda</button><button className={selectedScope === "left" ? "selected" : ""} onClick={() => selectAdditionScope(addition, "left")} type="button">Primera</button><button className={selectedScope === "right" ? "selected" : ""} onClick={() => selectAdditionScope(addition, "right")} type="button">Segunda</button></div> : null}<div className="public-addition-counter"><button aria-label={`Quitar ${addition.name}`} onClick={() => changeAddition(addition, selectedScope, -1)} type="button"><Minus size={15}/></button><b>{current}</b><button aria-label={`Agregar ${addition.name}`} onClick={() => changeAddition(addition, selectedScope, 1)} type="button"><Plus size={15}/></button></div></div>; })}</div></section> : <p className="public-empty">Sin extras compatibles para esta pizza.</p> : <section className="public-personalization-section public-personalization-panel">{removable.length ? <><div className="public-section-title"><span>Personaliza a tu gusto</span><h3>Quitar ingredientes</h3></div><div className="public-ingredient-list">{removable.map((ingredient) => <button className={removed.some((item) => item.source_id === ingredient.source_id && item.source_kind === ingredient.source_kind) ? "removed" : ""} key={`${ingredient.source_kind}-${ingredient.source_id}`} onClick={() => toggleRemoved(ingredient)} type="button"><Check size={15}/><span>{ingredient.name}</span></button>)}</div></> : <p className="public-empty">Esta pizza no tiene ingredientes removibles.</p>}</section>}</div>;
    if (isBeverageStep) return <section className="public-drink-step"><div className="public-section-title"><span>Completa tu pedido</span><h3>Elige bebidas o productos</h3><p>Es completamente opcional. Puedes continuar sin agregar nada.</p></div>{availableDrinks.length ? <div className="public-drink-grid">{availableDrinks.map((product) => { const current = drinkQuantities[product.id] ?? 0; return <article key={product.id}>{product.image_url ? <img alt="" src={product.image_url}/> : <span>{product.name.slice(0, 2)}</span>}<div><strong>{product.name}</strong><small>{presentation(product)} · {formatCop(product.sale_price_cop)}</small></div><div className="public-addition-counter"><button aria-label={`Quitar ${product.name}`} onClick={() => changeDrink(product, -1)} type="button"><Minus size={15}/></button><b>{current}</b><button aria-label={`Agregar ${product.name}`} onClick={() => changeDrink(product, 1)} type="button"><Plus size={15}/></button></div></article>; })}</div> : <p className="public-empty">No hay bebidas disponibles en este momento.</p>}</section>;
    return <div className="public-pizza-review"><div>{selected.image_url ? <img alt={selected.flavor_name} src={selected.image_url}/> : <div className="public-pizza-art"/>}{second?.image_url ? <img alt={second.flavor_name} src={second.image_url}/> : null}</div><section><span>{mode === "half" ? "Mitad y mitad" : "Pizza entera"}</span><h3>{selected.flavor_name}{second ? ` / ${second.flavor_name}` : ""}</h3><p>{selected.size_name}</p>{removed.length ? <p>Sin: {removed.map((item) => item.name).join(", ")}</p> : <p>Sin cambios en ingredientes</p>}{additions.length ? additions.map((addition) => <small key={`${addition.id}:${addition.scope}`}>+ {addition.name} x{addition.quantity}{addition.scope_label ? ` (${addition.scope_label})` : ""}</small>) : null}{selectedDrinks.length ? <p className="public-review-drinks">Bebidas: {selectedDrinks.map(({ product, quantity }) => `${product.name} x${quantity}`).join(", ")}</p> : null}<div className="public-review-quantity"><button onClick={() => setQuantity((value) => Math.max(1, value - 1))} type="button"><Minus/></button><b>{quantity}</b><button onClick={() => setQuantity((value) => value + 1)} type="button"><Plus/></button><strong>{formatCop(totalToCart)}</strong></div><button className="public-add-drink" onClick={openBeverageStep} type="button"><span aria-hidden="true">🥤</span><span><b>COMPLETA TU PEDIDO</b>{selectedDrinks.length ? "Cambiar bebidas" : "Agregar bebida"} <ChevronRight size={16}/></span></button></section></div>;
  }
  ...
  return <div className="public-overlay"><section aria-label="Configura tu pizza" aria-modal="true" className="public-configurator" role="dialog"><header><div><span>Paso {step} de {totalSteps}</span><h2>{title}</h2></div><button aria-label="Cerrar" onClick={onClose} type="button"><X/></button></header><div className="public-step-progress">{Array.from({ length: totalSteps }, (_, index) => <i className={index < step ? "active" : ""} key={index}/>)}</div>{summary}<div className="public-config-body">{renderStep()}</div><footer>{step > 1 ? <button className="public-secondary public-back" onClick={previousStep} type="button">Atras</button> : <span/>}{isReview ? <button className="public-primary" onClick={finishPizza} type="button">Agregar al carrito · {formatCop(totalToCart)}</button> : <button className="public-primary" disabled={mode === "half" && step === 3 && !second} onClick={nextStep} type="button">Continuar <ChevronRight size={18}/></button>}</footer></section></div>;
}
