...
begin
  ...
  with sellable_references as (
    ...
    join public.inventory_items master
      on master.item_kind = 'sale_product' and master.presentation_quantity is null and master.is_active
      and upper(master.name) = upper(refs.name)
    where refs.item_kind = 'sale_product' and refs.presentation_quantity is not null and refs.presentation_unit is not null
      and refs.is_active and refs.sale_is_enabled and refs.sale_price_cop > 0
  ...
  availability as (
    ...
    from purchase_balances group by inventory_item_id
  )
