...
declare
  ...
  item_notes text;
  second_flavor_id uuid := null;
  second_menu_category_id uuid := null;
  second_sale_price_cop numeric := null;
  second_sku text := null;
  second_flavor_name text := null;
  second_size_id uuid := null;
begin
  ...
  loop
    ...
    if item_kind_value = 'pizza' then
      ...
      second_flavor_id := null;
      second_menu_category_id := null;
      second_sale_price_cop := null;
      second_sku := null;
      second_flavor_name := null;
      second_size_id := null;
      ...
      if secondary_item_id is not null then
        ...
        into
          second_flavor_id,
          second_menu_category_id,
          second_sale_price_cop,
          second_sku,
          second_flavor_name,
          second_size_id
        from public.pizza_price_configs ppc
      ...
      if secondary_item_id is not null then
        line_price := greatest(pizza_row.sale_price_cop, second_sale_price_cop);
        item_name_snapshot := pizza_row.flavor_name || ' / ' || second_flavor_name || ' ' || pizza_row.size_name;
        item_sku_snapshot := case when second_sale_price_cop > pizza_row.sale_price_cop then second_sku else pizza_row.sku end;
      ...
      insert into public.pos_order_items (
        ...
        sku_snapshot, unit_price_cop, line_subtotal_cop, line_cost_cop, cart_line_key, notes
      )
      values (
        new_order_id, 'pizza', item_id, item_quantity, item_name_snapshot,
        item_sku_snapshot, line_price, item_subtotal, 0, nullif(item->>'line_key', ''), item_notes
      )
      returning id into inserted_item_id;

      select *
      into pizza_base_row
      from public.pos_resolve_pizza_base(pizza_row.size_id, item);

      update public.pos_order_items
      set base_default_source_kind = pizza_base_row.default_source_kind,
          base_default_inventory_item_id = pizza_base_row.default_inventory_item_id,
          base_default_preparation_id = pizza_base_row.default_preparation_id,
          base_used_source_kind = pizza_base_row.source_kind,
          base_used_inventory_item_id = pizza_base_row.inventory_item_id,
          base_used_preparation_id = pizza_base_row.source_preparation_id,
          base_used_quantity_base = pizza_base_row.quantity_base * item_quantity,
          base_used_unit = pizza_base_row.unit,
          base_replaced_by = case
            when pizza_base_row.source_kind is distinct from pizza_base_row.default_source_kind
              or coalesce(pizza_base_row.inventory_item_id, pizza_base_row.source_preparation_id)
                 is distinct from coalesce(pizza_base_row.default_inventory_item_id, pizza_base_row.default_preparation_id)
            then auth.uid() else null end,
          base_replaced_at = case
            when pizza_base_row.source_kind is distinct from pizza_base_row.default_source_kind
              or coalesce(pizza_base_row.inventory_item_id, pizza_base_row.source_preparation_id)
                 is distinct from coalesce(pizza_base_row.default_inventory_item_id, pizza_base_row.default_preparation_id)
      ...
      loop
        ...
        if secondary_item_id is not null then
          if exists (select 1 from public.pizza_addition_flavors where addition_id = addition_row.id)
            and not exists (
              select 1 from public.pizza_addition_flavors
              where addition_id = addition_row.id and flavor_id = second_flavor_id
            ) then
        ...
        if secondary_item_id is not null then
          if exists (select 1 from public.pizza_addition_categories where addition_id = addition_row.id)
            and (
              second_menu_category_id is null
              or not exists (
                select 1 from public.pizza_addition_categories
                where addition_id = addition_row.id and menu_category_id = second_menu_category_id
              )
            ) then
