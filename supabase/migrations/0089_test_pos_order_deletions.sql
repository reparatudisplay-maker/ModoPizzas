-- Test-only POS order deletion. This deliberately preserves a compact audit trail
-- and keeps unreintegrated consumption represented as an existing inventory waste.

alter table public.site_settings
  add column if not exists test_order_deletion_enabled boolean not null default true;

create table if not exists public.pos_order_test_deletion_audits (
  id uuid primary key default gen_random_uuid(),
  deleted_order_id uuid not null unique,
  order_code text not null,
  order_total_cop numeric(14,2) not null,
  order_snapshot jsonb not null,
  payment_snapshot jsonb not null default '[]'::jsonb,
  cash_movement_snapshot jsonb not null default '[]'::jsonb,
  operational_date_snapshot jsonb not null default '[]'::jsonb,
  reason_code text null check (reason_code in ('loss_total', 'prepared_product', 'damaged_product', 'system_test', 'other')),
  reason_detail text null,
  reintegrated_allocation_count integer not null default 0 check (reintegrated_allocation_count >= 0),
  retained_allocation_count integer not null default 0 check (retained_allocation_count >= 0),
  deleted_by uuid not null references auth.users(id) on delete restrict,
  deleted_at timestamptz not null default now()
);

create table if not exists public.pos_order_test_deletion_consumptions (
  id uuid primary key default gen_random_uuid(),
  audit_id uuid not null references public.pos_order_test_deletion_audits(id) on delete cascade,
  original_allocation_id uuid not null unique,
  consumption_snapshot jsonb not null,
  was_reintegrated boolean not null,
  physical_inventory_count_id uuid null references public.physical_inventory_counts(id) on delete restrict,
  created_at timestamptz not null default now()
);

create index if not exists pos_order_test_deletion_audits_deleted_at_idx
  on public.pos_order_test_deletion_audits (deleted_at desc);

create index if not exists pos_order_test_deletion_consumptions_audit_idx
  on public.pos_order_test_deletion_consumptions (audit_id);

alter table public.pos_order_test_deletion_audits enable row level security;
alter table public.pos_order_test_deletion_consumptions enable row level security;

drop policy if exists "Managers can read test order deletion audits" on public.pos_order_test_deletion_audits;
create policy "Managers can read test order deletion audits"
  on public.pos_order_test_deletion_audits for select
  to authenticated
  using (app_private.has_any_role(array['gerente'::public.app_role, 'admin_sistema'::public.app_role]));

drop policy if exists "Managers can read test order deletion consumptions" on public.pos_order_test_deletion_consumptions;
create policy "Managers can read test order deletion consumptions"
  on public.pos_order_test_deletion_consumptions for select
  to authenticated
  using (app_private.has_any_role(array['gerente'::public.app_role, 'admin_sistema'::public.app_role]));

-- Do not expose direct mutations of deletion audits. The atomic RPC below is the
-- only supported writer and is guarded by role plus the central test-mode switch.
revoke all on public.pos_order_test_deletion_audits from anon, authenticated;
revoke all on public.pos_order_test_deletion_consumptions from anon, authenticated;
grant select on public.pos_order_test_deletion_audits, public.pos_order_test_deletion_consumptions to authenticated;

create or replace function public.get_pos_order_test_deletion_preview(p_order_id uuid)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'allocation_id', poca.id,
    'consumption_id', poc.id,
    'order_item_id', poc.order_item_id,
    'order_item_addition_id', poc.order_item_addition_id,
    'item_name', coalesce(addition.name_snapshot, item.product_name_snapshot, 'Consumo sin línea'),
    'item_kind', item.item_kind,
    'quantity_base', poca.quantity_base,
    'base_unit', poca.base_unit,
    'cost_cop', poca.cost_cop,
    'source_kind', poc.source_kind,
    'source_name', coalesce(inventory.name, preparation.name, 'Fuente sin registro'),
    'purchase_item_id', poca.purchase_item_id,
    'production_batch_id', poca.production_batch_id,
    'origin_label', case
      when poca.purchase_item_id is not null then concat('Compra ', to_char(purchase.purchased_at at time zone 'America/Bogota', 'DD/MM/YYYY'), ' · lote ', left(purchase_item.id::text, 8))
      when poca.production_batch_id is not null then concat('Producción P', production_batch.production_number::text, ' · ', to_char(production_batch.elaborated_at, 'DD/MM/YYYY'))
      else 'Origen sin registro'
    end,
    'combo_name', combo.name
  ) order by combo.name nulls last, item.product_name_snapshot, poca.created_at), '[]'::jsonb)
  into consumption_rows
  from public.pos_order_consumption_allocations poca
  join public.pos_order_consumptions poc on poc.id = poca.consumption_id
  left join public.pos_order_items item on item.id = poc.order_item_id
  left join public.pos_order_item_additions addition on addition.id = poc.order_item_addition_id
  left join public.inventory_items inventory on inventory.id = poc.inventory_item_id
  left join public.preparations preparation on preparation.id = poc.source_preparation_id
  left join public.purchase_items purchase_item on purchase_item.id = poca.purchase_item_id
  left join public.purchases purchase on purchase.id = purchase_item.purchase_id
  left join public.production_batches production_batch on production_batch.id = poca.production_batch_id
  left join lateral (
    select snapshot.name
    from public.pos_order_combo_snapshots snapshot
    where snapshot.order_id = p_order_id
      and snapshot.cart_line_key = item.cart_line_key
    order by snapshot.created_at desc
    limit 1
  ) combo on true
  where poc.order_id = p_order_id;
    ...
    if allocation_row.allocation_id = any(coalesce(retained_ids, '{}'::uuid[])) then
      ...
      if allocation_row.source_kind = 'inventory_item' then
        select
          ...
          + coalesce((
            select sum(case
              when count_row.difference_quantity_base > 0 then public.production_convert_quantity(count_row.difference_quantity_base, count_row.base_unit, allocation_row.base_unit, null)
              when count_row.difference_quantity_base < 0 then -public.production_convert_quantity(-count_row.difference_quantity_base, count_row.base_unit, allocation_row.base_unit, null)
              else 0
            end)
...
revoke all on function public.get_pos_order_test_deletion_preview(uuid) from public;
grant execute on function public.get_pos_order_test_deletion_preview(uuid) to authenticated;
